...
import { FiPlus, FiSearch, FiFilter, FiDownload, FiEdit2, FiEye, FiTrash2, FiChevronLeft, FiChevronRight, FiX, FiUser, FiMail, FiPhone, FiBriefcase, FiCalendar } from 'react-icons/fi';
import { useCartoes } from '../contextos/CartoesContext';
import TabelaFuncionarios from '../componentes/funcionarios/TabelaFuncionarios';
...
const Funcionarios = () => {
  ...
  const funcionariosFiltrados = funcionarios
    .filter((func) => {
      ...
      switch(campoBusca) {
        case 'nome':
          return func.nomeCompleto.toLowerCase().includes(termoBusca);
        case 'email':
          return func.email?.toLowerCase().includes(termoBusca);
        case 'telefone':
          return func.telefone?.includes(busca);
        case 'empresa':
          return func.empresa?.toLowerCase().includes(termoBusca);
        case 'cargo':
          return func.cargo?.toLowerCase().includes(termoBusca);
        case 'todos':
        default:
          return (
  ...
  const funcionariosPaginados = funcionariosFiltrados.slice(indexPrimeiroItem, indexUltimoItem);
  ...
  const handleExcluir = (funcionario) => {
    if (window.confirm(`Tem certeza que deseja excluir ${funcionario.nomeCompleto}?`)) {
  ...
  const exportarDados = () => {
    const dadosCSV = funcionariosFiltrados.map(func => ({
      Nome: func.nomeCompleto,
      Email: func.email || '',
      Telefone: func.telefone || '',
      Empresa: func.empresa || '',
      Cargo: func.cargo || '',
      Data: new Date(func.dataCriacao).toLocaleDateString('pt-BR')
    }));
        ...
        <div className="flex flex-wrap gap-3 items-center">
          <div className="flex items-center gap-2 text-sm">
            <span className="bg-gray-100 px-3 py-1 rounded-full">
              <span className="font-semibold">{funcionariosFiltrados.length}</span> funcionário(s)
            </span>
          ...
          <div className="flex flex-wrap gap-2">
            {busca && (
              <span className="bg-[#106a37]/10 text-[#106a37] px-3 py-1 rounded-full text-sm flex items-center gap-1">
                Busca: "{busca}"
                <button onClick={() => setBusca('')} className="ml-1 hover:text-[#0d5a2c]">
                  <FiX size={14} />
                </button>
              </span>
            )}
            {filtroAtivo !== 'todos' && (
              <span className="bg-[#106a37]/10 text-[#106a37] px-3 py-1 rounded-full text-sm flex items-center gap-1">
                {filtroAtivo === 'comEmpresa' ? 'Com empresa' : 
                 filtroAtivo === 'semEmpresa' ? 'Sem empresa' :
                 filtroAtivo === 'comFoto' ? 'Com foto' :
                 filtroAtivo === 'semFoto' ? 'Sem foto' :
                 filtroAtivo === 'comTelefone' ? 'Com telefone' : 'Sem telefone'}
                <button onClick={() => setFiltroAtivo('todos')} className="ml-1 hover:text-[#0d5a2c]">
                  <FiX size={14} />
                </button>
              </span>
            )}
                  ...
                  {funcionariosPaginados.length > 0 ? (
                    funcionariosPaginados.map((funcionario, index) => (
                      ...
                      >
                        ...
                        <td className="py-4 px-6">
                          <div className={`px-3 py-1.5 rounded-full text-sm font-medium ${funcionario.empresa ? 'bg-[#106a37]/10 text-[#106a37]' : 'bg-gray-100 text-gray-600'}`}>
                            {funcionario.empresa || 'Não definida'}
                          </div>
                        </td>
                        <td className="py-4 px-6">
                          <p className="text-gray-700 font-medium">{funcionario.cargo || 'Não definido'}</p>
                        </td>
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-2">
                            <FiPhone className="text-gray-400" size={14} />
                            <p className="text-gray-700 font-medium">{funcionario.telefone || 'Não informado'}</p>
                          </div>
                        </td>
                        <td className="py-4 px-6">
                          <div className="flex flex-col">
                            ...
                            <p className="text-xs text-gray-500">
                              {new Date(funcionario.dataCriacao).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
                            </p>
                          </div>
                        </td>
                        <td className="py-4 px-6">
                      ...
                      <td colSpan="6" className="py-12 px-6 text-center">
                        <div className="flex flex-col items-center justify-center text-gray-500">
                          ...
                          <p className="text-sm mt-1">
                            {busca || filtroAtivo !== 'todos' ? 'Tente ajustar os filtros de busca' : 'Adicione seu primeiro funcionário'}
                          </p>
              ...
              <div className="border-t border-gray-100 px-6 py-4">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                  <div className="text-sm text-gray-600">
                    Mostrando <span className="font-semibold">{indexPrimeiroItem + 1}</span> a <span className="font-semibold">{Math.min(indexUltimoItem, funcionariosFiltrados.length)}</span> de <span className="font-semibold">{funcionariosFiltrados.length}</span> funcionários
                  </div>
